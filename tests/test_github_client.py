...
class TestGitHubClient(unittest.TestCase):
    ...
    def test_fetch_commits(self, mock_session_class):
        """
        ...
        commits = client.fetch_commits(self.repo)
    ...
    @patch('github_client.requests.Session')
    def test_fetch_issues(self, mock_session_class):
        """
        ...
        issues = client.fetch_issues(self.repo)
    ...
    @patch('github_client.requests.Session')
    def test_fetch_pull_requests(self, mock_session_class):
        """
        ...
        pull_requests = client.fetch_pull_requests(self.repo)
    ...
    @patch('github_client.requests.Session')
    def test_export_daily_progress(self, mock_session_class):
        """
        ...
        file_path = client.export_daily_progress(self.repo)
        self.assertTrue(file_path.endswith('.md'))  # 检查生成的文件路径是否以 .md 结尾

    @patch('github_client.requests.Session')
    def test_export_progress_by_date_range(self, mock_session_class):
        """
        ...
        file_path = client.export_progress_by_date_range(self.repo, days=7)
        self.assertTrue(file_path.endswith('.md'))  # 检查生成的文件路径是否以 .md 结尾
