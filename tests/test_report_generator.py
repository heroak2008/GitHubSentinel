...
class TestReportGenerator(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_prompts = {
            ...
            "twitter_ai_news": "Twitter AI news specific prompt..."
        }
        ...
        self.test_twitter_ai_file_path = 'test_twitter_ai.md'
        ...
        with open(self.test_twitter_ai_file_path, 'w', encoding='utf-8') as file:
            file.write(self.markdown_content)
    ...
    def tearDown(self):
        ...
        if os.path.exists(self.test_twitter_ai_file_path):
            os.remove(self.test_twitter_ai_file_path)
        ...
        twitter_ai_report_path = os.path.splitext(self.test_twitter_ai_file_path)[0] + "_report.md"
        if os.path.exists(twitter_ai_report_path):
            os.remove(twitter_ai_report_path)
    ...
    @patch.object(ReportGenerator, '_preload_prompts', return_value=None)
    def test_generate_twitter_ai_report(self, mock_preload_prompts):
        """
        测试 generate_twitter_ai_report 方法是否正确生成报告并保存到文件。
        """
        # 初始化 ReportGenerator 实例，并手动设置 prompts
        self.report_generator = ReportGenerator(self.mock_llm, ["github", "hacker_news_hours_topic", "hacker_news_daily_report", "twitter_ai_news"])
        self.report_generator.prompts = self.mock_prompts

        # 模拟 LLM 返回的报告内容
        mock_report = "This is a generated Twitter AI news report."
        self.mock_llm.generate_report.return_value = mock_report

        # 调用 generate_twitter_ai_report 方法
        report, report_file_path = self.report_generator.generate_twitter_ai_report(self.test_twitter_ai_file_path)
        ...
        # 验证 LLM 的 generate_report 方法是否被正确调用，且传入了正确的参数
        self.mock_llm.generate_report.assert_called_once_with(self.mock_prompts["twitter_ai_news"], self.markdown_content)
